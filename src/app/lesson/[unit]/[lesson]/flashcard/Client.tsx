  ...
  return (
    <ExerciseShell title="Flashcard" backHref={`/lesson/${unit}/${lesson}`} current={index} total={words.length}>
      <div className="flex-1 flex flex-col gap-4 pt-4">
        ...
        >
          {!flipped ? (
            <><p className="text-xs text-gray-400 mb-2">English — tap to reveal</p><p className="text-2xl font-semibold text-gray-800">{word.translation}</p></>
          ) : (
            <><p className="text-xs text-gray-400 mb-2">Latvian</p><p className="text-2xl font-bold text-amber-600">{word.original}</p><div className="flex justify-center mt-3"><AudioButton src={`/audio/${word.soundFile}`} size="md" autoPlay /></div></>
          )}
        </div>
        {flipped && (
          <div className="grid grid-cols-2 gap-3 mt-2">
            <button onClick={() => next(false)} className="py-4 rounded-2xl bg-red-100 text-red-600 font-semibold text-lg active:scale-95">✗ Wrong</button>
            <button onClick={() => next(true)} className="py-4 rounded-2xl bg-green-100 text-green-600 font-semibold text-lg active:scale-95">✓ Got it</button>
          </div>
        )}
      </div>
    </ExerciseShell>
